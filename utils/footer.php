<hr class = "footerline"><!--css modified horizontal line-->
<footer>
    <div class = "container">
        <div class = "row">
            <section>
                <div class = "footerContent col-md-4"><!--left content-->
                    <p class = "footerContent1">
                        <strong>S</strong><span class = "small footerSubtext">akshi</span>
                        <strong>M</strong><span class = "small footerSubtext">anmode's</span>
                        <strong>E</strong><span class = "small footerSubtext">vents</span>
                    
                    </p>

                    <p class = "footerSubtext2">
                        Kill Avenue, Dun Laoghaire, Co. Dublin
                        &copy; Sakshi Events Venue & Catering, 2021.
                    </p>
                </div>
            </section>
            <section>
                <div class = "footcontent col-md-4"><!--middle content-->
                    DL824 Multimedia Design/Web Engineering<br>
                    Creative Computing 2021-22.<br>
                    Julz Engracio, N00145647
                </div>
            </section>
            <section>
                <div class = "footcontent col-md-4"><!--right content-->
                    Follow Us:<br>
                        <img src = "images/facebook.png">
                        <img src = "images/twitter.png">
                        <img src = "images/googleplus.png">
                        <img src = "images/youtube.png">
                </div>
            </section>
        </div>
    </div>
</footer>